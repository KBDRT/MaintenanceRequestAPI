...
describe(`Изменение статуса заявки: PATCH ${PATH}/status`, () => {
  ...
  test('200 - валидные данные', async () => {
    ...
    const getResponse = await request(app).patch(`${PATH}/${id}/status`).send(status);
    expect(getResponse.status).toBe(204);
  });

  test('409 - запрещенный переход', async () => {
    const equipmentBody = createEquipmentBody();
  ...
  test('404 - не найдено', async () => {
    ...
    const response = await request(app).patch(`${PATH}/${id}/status`).send(status);
